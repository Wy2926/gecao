...
**测试分工（已定）**：**自动化逻辑测试全部由开发方（Devin）负责并随 CI 跑**；**所有需要人工的体验/手感验收交给用户**，开发方提供验收清单 + 在线可试玩链接。
- **单测（Vitest，开发方）**：伤害乘区公式、暴击/护甲、StatSheet 叠加与上限、异常叠层/爆发、抽卡权重分布/前置/互斥、RNG 种子可复现、联动事件触发链、轨迹原语、进化规则。
- **冒烟/集成（开发方，headless）**：无渲染逻辑跑「打一局」脚本，断言能升级、能触发至少 1 条异常联动连锁（对应 08 验收）。
- **手动测试（交用户）**：手感、打击感、美术/动画观感、UI 体验、整体节奏等主观项，由用户在每里程碑的在线链接上验收并反馈；开发方据此迭代。
- CI：`lint` + `typecheck` + `vitest`；pre-commit 跑 lint-staged。每个里程碑产出可试玩链接交付用户手动验收。
