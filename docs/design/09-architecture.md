...
### 4.2 机制
- 每张联动/增益卡 = 一个 `AugmentHandler`：声明监听哪个事件 + 执行函数（概率判定走可注入 RNG）。
- 触发型联动（天降流星/连锁闪电）需 **内部冷却（ICD 0.3~0.5s）** 防刷屏（06 待定项已预留字段）。
- 效果若产生新伤害（尸爆/连锁/分裂），作为**独立伤害事件**重新进入结算管线 → 自然形成连锁，但用「连锁深度上限」防无限递归。
- 这样新增一张联动卡 = 在 `config/augments.ts` 增一条 + 写一个 handler，**不动核心战斗**。
...
1. **实体组织方式**：建议「**轻量组件式**（实体持有 StatSheet/StatusController/AI 等组件）」而非引入完整 ECS 库——更贴合 Phaser，复杂度可控。是否认可？
2. **数据表载体**：建议**直接用 TS 配置对象**（强类型、可引用枚举/标签）而非外部 JSON。是否认可？
3. **包管理器**：`pnpm` vs `npm`，是否有偏好？
4. **部署目标**：MVP 阶段是否要我配置静态部署（如 GitHub Pages / devinapps）以便随时在线试玩？
5. **本阶段产物**：按 `00` 决策「先只出设计文档」，本文也属设计文档；**是否现在就进入 M0 脚手架编码**，还是先评审本架构再开工？
